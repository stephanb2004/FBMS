package net.mcreator.fbms.block.renderer;

import software.bernie.geckolib.renderer.GeoBlockRenderer;

import net.minecraft.resources.ResourceLocation;
import net.minecraft.client.renderer.RenderType;
import net.minecraft.client.renderer.MultiBufferSource;

import net.mcreator.fbms.block.model.PizzaBox93ClosedBlockModel;
import net.mcreator.fbms.block.entity.PizzaBox93ClosedTileEntity;

public class PizzaBox93ClosedTileRenderer extends GeoBlockRenderer<PizzaBox93ClosedTileEntity> {
	public PizzaBox93ClosedTileRenderer() {
		super(new PizzaBox93ClosedBlockModel());
	}

	@Override
	public RenderType getRenderType(PizzaBox93ClosedTileEntity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
		return RenderType.entityTranslucent(getTextureLocation(animatable));
	}
}

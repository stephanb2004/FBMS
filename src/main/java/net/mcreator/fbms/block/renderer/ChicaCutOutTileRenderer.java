package net.mcreator.fbms.block.renderer;

import software.bernie.geckolib.renderer.GeoBlockRenderer;

import net.minecraft.resources.ResourceLocation;
import net.minecraft.client.renderer.RenderType;
import net.minecraft.client.renderer.MultiBufferSource;

import net.mcreator.fbms.block.model.ChicaCutOutBlockModel;
import net.mcreator.fbms.block.entity.ChicaCutOutTileEntity;

public class ChicaCutOutTileRenderer extends GeoBlockRenderer<ChicaCutOutTileEntity> {
	public ChicaCutOutTileRenderer() {
		super(new ChicaCutOutBlockModel());
	}

	@Override
	public RenderType getRenderType(ChicaCutOutTileEntity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
		return RenderType.entityTranslucent(getTextureLocation(animatable));
	}
}

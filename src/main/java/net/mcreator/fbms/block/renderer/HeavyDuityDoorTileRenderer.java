package net.mcreator.fbms.block.renderer;

import software.bernie.geckolib.renderer.GeoBlockRenderer;

import net.minecraft.resources.ResourceLocation;
import net.minecraft.client.renderer.RenderType;
import net.minecraft.client.renderer.MultiBufferSource;

import net.mcreator.fbms.block.model.HeavyDuityDoorBlockModel;
import net.mcreator.fbms.block.entity.HeavyDuityDoorTileEntity;

public class HeavyDuityDoorTileRenderer extends GeoBlockRenderer<HeavyDuityDoorTileEntity> {
	public HeavyDuityDoorTileRenderer() {
		super(new HeavyDuityDoorBlockModel());
	}

	@Override
	public RenderType getRenderType(HeavyDuityDoorTileEntity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
		return RenderType.entityTranslucent(getTextureLocation(animatable));
	}
}
